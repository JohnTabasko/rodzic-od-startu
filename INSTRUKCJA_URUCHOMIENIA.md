...
```bash
npm install        # pobiera zależności (~1-2 min)
...
Edytuj **jedną linię** — `src/services/api.ts`:
```ts
export const API_URL = 'http://TU_WPISZ:3000/api';
```
...
| Błąd o module natywnym (SecureStore/AV/Notifications) | Expo Go nie ma modułu | development build (KROK 5); apka z Expo Go działa w trybie okrojonym |
...
```bash
# APLIKACJA
npm install && npm start                    # start dev
